...
cp -an "$1/device/motorola/quark/multirom/libutils.so" "$2/"
cp -an "$1/device/motorola/quark/multirom/libstlport.so" "$2/"

mkdir -p "$2/vendor/firmware/keymaster"
mkdir -p "$2/vendor/lib/hw/"
cp -a "$1/device/motorola/quark/recovery/root/vendor/lib/hw/keystore.default.so" "$2/vendor/lib/hw/keystore.default.so"
